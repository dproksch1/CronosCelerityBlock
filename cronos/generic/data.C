...
// void Pot::set_FieldType(string name) {
// 	this->FieldType = name;
// }

// string Pot::get_FieldType() {
// 	// Get name of variable used in program
// 	return FieldType;
// }

/*-----------------------------------------------------------------
  Data container -- in current version just holding the actual data
  -----------------------------------------------------------------*/
...
//	cout << " Making new class " << endl << endl << endl;
#if (FLUID_TYPE == CRONOS_MULTIFLUID)

	int nFluids = NUM_FLUIDS;
	fluids = new CronosMultifluid(nFluids, ENERGETICS, OMS_USER);
#if(CRSWITCH_DUAL_ENERGY == CRONOS_ON)
	fluids->set_dualEnergy(AUX_ENERGY);
#else
	fluids->unset_dualEnergy();
#endif
	fluids->setup();
	fluids->compute_Variables(N_ADD, N_SUBS);
	int N_OM = fluids->get_N_OM();
	int n_omInt = fluids->get_N_OMINT();
	int n_omUser = fluids->get_N_OM_USER();

#else // Now: single-fluid case
...
#if (OMS_USER == TRUE)
#if (FLUID_TYPE == CRONOS_MULTIFLUID)
	int N_OM_USER = fluids->get_N_OM_USER();
	int n_omIntUser = fluids->get_N_OMINT_USER();
#else
...
#ifdef parallel
    double radialArea_global;
	MPI_Reduce(&radialArea, &radialArea_global, 1, MPI_DOUBLE,
			MPI_SUM, 0, comm_constx);
	MPI_Bcast(&radialArea_global, 1, MPI_DOUBLE, 0, comm_constx);
//	MPI_Barrier(comm_constx);
	radialArea = radialArea_global;
#endif
...
void Data::MpiSum(double &variable) {
	double variableGlobal = variable;
	MPI_Reduce(&variable, &variableGlobal, 1, MPI_DOUBLE,
	           MPI_SUM, 0, comm3d);
	MPI_Bcast(&variableGlobal, 1, MPI_DOUBLE, 0, comm3d);
//	MPI_Barrier(comm3d);
	variable = variableGlobal;
}


void Data::MpiSum(int &variable) {
	int variableGlobal = variable;
	MPI_Reduce(&variable, &variableGlobal, 1, MPI_INT,
	           MPI_SUM, 0, comm3d);
	MPI_Bcast(&variableGlobal, 1, MPI_INT, 0, comm3d);
//	MPI_Barrier(comm3d);
	variable = variableGlobal;
}


void Data::MpiSum(REAL variable[3]) {
	REAL variableGlobal[3] = {variable[0], variable[1], variable[2]};
	MPI_Reduce(&variable[0], &variableGlobal[0], 3, MPI_DOUBLE,
	           MPI_SUM, 0, comm3d);
	MPI_Bcast(&variableGlobal[0], 3, MPI_DOUBLE, 0, comm3d);
//	MPI_Barrier(comm3d);
	variable[0] = variableGlobal[0];
	variable[1] = variableGlobal[1];
	variable[2] = variableGlobal[2];
}


void Data::MpiSum(REAL variable[], int num) {
	//REAL variableGlobal[num];
	std::vector<REAL> variableGlobal(num);
	for(int i=0; i<num; ++i) {
		variableGlobal[i] = variable[i];
	}
//	MPI_Barrier(comm3d);
	MPI_Reduce(&variable[0], &variableGlobal[0], num, MPI_DOUBLE,
	           MPI_SUM, 0, comm3d);
	MPI_Bcast(&variableGlobal[0], num, MPI_DOUBLE, 0, comm3d);
//	MPI_Barrier(comm3d);
	for(int i=0; i<num; ++i) {
		variable[i] = variableGlobal[i];
	}
}

void Data::MpiSum(int dir, int &variable) {
	int variableGlobal = variable;
	if(dir == 0) { // Use plane in x-dir:
		MPI_Reduce(&variable, &variableGlobal, 1, MPI_INT,
		           MPI_SUM, 0, comm_constx);
 		MPI_Bcast(&variableGlobal, 1, MPI_INT, 0, comm_constxy);
//		MPI_Barrier(comm_constx);
	} else if(dir == 2) { // Use plane in z-dir:
		MPI_Reduce(&variable, &variableGlobal, 1, MPI_INT,
		           MPI_SUM, 0, comm_constz);
		MPI_Bcast(&variableGlobal, 1, MPI_INT, 0, comm_constz);
//		MPI_Barrier(comm_constz);
	}
	variable = variableGlobal;
}

void Data::MpiSum(int dir, double &variable) {
	double variableGlobal = variable;
	if(dir == 0) { // Use plane in x-dir:
		MPI_Reduce(&variable, &variableGlobal, 1, MPI_DOUBLE,
		           MPI_SUM, 0, comm_constx);
 		MPI_Bcast(&variableGlobal, 1, MPI_DOUBLE, 0, comm_constxy);
//		MPI_Barrier(comm_constx);
	} else if(dir == 2) { // Use plane in z-dir:
		MPI_Reduce(&variable, &variableGlobal, 1, MPI_DOUBLE,
		           MPI_SUM, 0, comm_constz);
		MPI_Bcast(&variableGlobal, 1, MPI_DOUBLE, 0, comm_constz);
//		MPI_Barrier(comm_constz);
	}
	variable = variableGlobal;
}
...
#ifdef parallel
	int found_NAN_global;
	MPI_Allreduce(&found_NAN, &found_NAN_global, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
	found_NAN = found_NAN_global;
//	if(found_NAN) {
//		int qNAN_global, iNAN_global, jNAN_global, kNAN_global;
//
//		MPI_Allreduce(&qNAN, &qNAN_global, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
//		MPI_Allreduce(&iNAN, &iNAN_global, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
//		MPI_Allreduce(&jNAN, &jNAN_global, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
//		MPI_Allreduce(&kNAN, &kNAN_global, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
//
//		qNAN = qNAN_global;
//		iNAN = iNAN_global;
//		jNAN = jNAN_global;
...
#if(FLUID_TYPE==CRONOS_MULTIFLUID)
	int n_omInt = fluids->get_N_OMINT();
	int n_Omega = fluids->get_N_OMEGA();

	// Loop over normal fields
	for(int q=0; q<n_omInt; ++q) {
		string name = om[q].getName();
		int iFluid = fluids->get_FluidIndex(q);
		string nameFluid = fluids->fluids[iFluid].get_Name();
		name = nameFluid + "/" + name;
		cout << " name: " << name << " " << iFluid << endl;
		om_ids.enter_fieldId(name);
	}

	// Loop over extra-fields:
	for(int q=n_omInt; q<n_omInt+N_ADD+N_SUBS; ++q) {
		string name = om[q].getName();
		cout << " name var " << name << endl;
		om_ids.enter_fieldId(name);
	}

	// Loop over storage fields
	for(int q=0; q<n_omInt; ++q) {
		string name = om[q+n_Omega].getName();
		int iFluid = fluids->get_FluidIndex(q);
		string nameFluid = fluids->fluids[iFluid].get_Name();
		name = nameFluid + "/" + name;
		cout << " name var var " << name << endl;
		om_ids.enter_fieldId(name);
	}

...
#ifdef parallel
	MPI_Allreduce(MPI_IN_PLACE, &RunTimeExit, 1, MPI_C_BOOL, MPI_LOR, gdata.comm3d);
#endif
...
#ifdef parallel
	MPI_Allreduce(MPI_IN_PLACE, &Completed, 1, MPI_C_BOOL, MPI_LOR, gdata.comm3d);
#endif
...
#ifdef parallel
	MPI_Allreduce(MPI_IN_PLACE, &EndProgram_User, 1, MPI_C_BOOL, MPI_LOR, gdata.comm3d);
#endif
...
void Environment::compute_dt(Data &gdata, double factor)
{
  ...
  // Make sure each cpu uses the same time
#ifdef parallel
//  MPI_Barrier(gdata.comm3d);
  MPI_Bcast(&gdata.dt, 1, MPI_DOUBLE, 0, gdata.comm3d);
//  MPI_Barrier(gdata.comm3d);
#endif
}
...
// #ifdef parallel
// //	MPI_Barrier(gdata.comm3d);
// 	MPI_Bcast(&outputflag, 5, MPI_INT, 0, gdata.comm3d);
// //	MPI_Barrier(gdata.comm3d);
// #endif
...
//	MPI_Barrier(gdata.comm3d);
	...
	//if(gdata.tstep==1) {
	//	EndProgram = Finalize(gdata, "stopping for valgrind");
	//}
...
void Environment::WriteDivB(Data &gdata) {
  ...
  // Adding coordinates for parallel case:
	...
	for(int i=0; i<DIM-1;++i) {
		sprintf(ccoord,"%i",gdata.coords[i]);
		filename += ccoord;
		filename +="-";
	}
	sprintf(ccoord,"%i",gdata.coords[DIM-1]);
	filename += ccoord;
#endif
...
void Environment::Abort(Data &gdata, CException exep)
{
	...
	<< " MPI-Rank " << gdata.rank << ":"
#endif
	<< " Program aborted due to this error: " << endl;
...
//		numout = n_omIntAll + N_SUBS;
	...
	h5out = new Hdf5Stream(filename, numout, gdata.rank, true, gdata.comm3d);
#endif // Check of Output Version
#else
	h5out = new Hdf5Stream(filename, numout, gdata.rank);
#endif
#else

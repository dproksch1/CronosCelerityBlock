...
void HyperbolicSolver::get_Changes(const Data &gdata,
		...
		int ix, int iy, int iz, int dir, int Fluid_Type, int iFluid) const {
...
void get_Changes2(const Data &gdata,
		const num_fields_0D &numfM, const num_fields_0D &numfP, NumMatrix<REAL, 3> nom[],
	...
	for (int q = 0; q < num; ++q){
		...
		nom[q_global](ix, iy, iz) += get_Changes1DCart2(gdata, numfP.ptotal_num, numfM.ptotal_num, dir, iPos, q);
...
REAL get_Changes1DCart2(const Data &gdata,
		const NumArray<REAL> &fluxP, const NumArray<REAL> &fluxM,

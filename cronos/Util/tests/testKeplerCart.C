...
//	const Quantity &massA, const Quantity &massB, const Quantity &semiMajor_Axis,
	...
	std::ofstream pyfile;
	pyfile.open("Kepler.csv");
	pyfile << "#time_num, xA_num, yA_num, xB_num, yB_num"<< endl;


	hid_t hdf5file =  H5Fcreate("Kepler.h5",H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
	...
	for(int i_time=0; i_time<num_steps; ++i_time) {
		...
		cout << " getting pos " << endl;

		orbit.get_stellarPositionsCart(myNorm, posA_num, posB_num, time_num);
		...
		orbit.get_stellarVelocitiesCart(myNorm, veloA_num, veloB_num, time_num);
	...
	hid_t dataset_id = H5Dcreate2(group, "time", datatype, dataspace,
			H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
	H5Dwrite(dataset_id, datatype, H5S_ALL,
			H5S_ALL, H5P_DEFAULT, h5_time);
	hid_t dataset_id2 = H5Dcreate2(group, "xA", datatype, dataspace,
			H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
	H5Dwrite(dataset_id2, datatype, H5S_ALL,
			H5S_ALL, H5P_DEFAULT, h5_xA);
	hid_t dataset_id3 = H5Dcreate2(group, "yA", datatype, dataspace,
			H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
	H5Dwrite(dataset_id3, datatype, H5S_ALL,
			H5S_ALL, H5P_DEFAULT, h5_yA);
	hid_t dataset_id4 = H5Dcreate2(group, "xB", datatype, dataspace,
			H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
	H5Dwrite(dataset_id4, datatype, H5S_ALL,
			H5S_ALL, H5P_DEFAULT, h5_xB);
	hid_t dataset_id5 = H5Dcreate2(group, "yB", datatype, dataspace,
			H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
	H5Dwrite(dataset_id5, datatype, H5S_ALL,
			H5S_ALL, H5P_DEFAULT, h5_yB);
	...
	dataset_id = H5Dcreate2(group, "vAx", datatype, dataspace,
			H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
	H5Dwrite(dataset_id, datatype, H5S_ALL,
			H5S_ALL, H5P_DEFAULT, h5_vA_x);
	H5Dclose(dataset_id);

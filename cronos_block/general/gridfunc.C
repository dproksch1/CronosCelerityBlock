...
// 	void dataout2(Data &gdata,  Hdf5Stream &h5out, ProblemType & Problem, 
// 					int n_omInt, int n_omIntUser,
...
// 				Write3DMatrix2(h5out, dsetName, data, xmin, gdata.dx, group,-1, true);
...
// 					}
// 				}
// 				Write3DMatrix2(h5out,dsetName, data, xmin, gdata.dx, group, q_index,true);
// 			}
...
// 						}
// 					}
// 		Write3DMatrix2(h5out,"carbuncle_flag", data, xmin, gdata.dx, group, -1, true);
                              ...
                              int dir, int above, int q, int rim)
{
	...
	if(dir == 1) {
		if(above == 0) {
			...
			// 		if(iy >= mx[1] + iymin + rim && iy < mx[1] + 2*iymin + 2*rim) {
			// 			om_acc[ix][iy][iz] = om_acc[ix-1][iy][iz];
			// 		}
	...
	if(dir == 2) {
		if(above == 0) {
			...
			// 		if(iz >= mx[2] + izmin + rim && iz < mx[2] + 2*izmin + 2*rim) {
			// 			om_acc[ix][iy][iz] = om_acc[ix-1][iy][iz];
			// 		}
